...
# 2.只使用了Encoder段-文本分类BERT和图片分类VIT
# 3.只使用了Decoder端-生成类模型
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask):
        ...
        context, attn = ScaledDotProductAttention()(q_s, k_s, v_s, attn_mask)    # 当定义类class时，在class中没有出现需要说明的变量，而在forward中出现需要这些变量，那么就是Scale()(....)
...
def get_attn_pad_mask(seq_q, seq_k):
    ...
    pad_attn_mask = seq_k.data.eq(0).unsqueeze(1)  # batch_size x 1 x len_k, one is masking  结果中 true即看作1，false看作0
    return pad_attn_mask.expand(batch_size, len_q, len_k)  # batch_size x len_q x len_k  expand第二个维度，即len_k代表行，len_q代表列，batch_size代表这样的有多少个


## 3. PositionalEncoding 代码实现
class PositionalEncoding(nn.Module):   ## PositionalEncoding是nn.Module的子类
    def __init__(self, d_model, dropout=0.1, max_len=5000):
        ...
        position = torch.arange(0, max_len, dtype=torch.float).unsqueeze(1)   ## unsqyeeze在对应位置增加一个维度   shape: [max_len,1]

        ## 假设我的demodel是512，2i那个符号中i从0取到了255，那么2i对应取值就是0,2,4...510
        div_term = torch.exp(torch.arange(0, d_model, 2).float() * (-math.log(10000.0) / d_model))          ## shape: [d_model]

        ## position * div_term  shape[max_len,d_model] 相当于每一行的position乘以div_term     【5*1】 * 【1*3】  =  【5*3】
        ...
        ## transpose交换指定的两个维度，下面这个代码之后，我们得到的pe形状是：[max_len，1，d_model]
        pe = pe.unsqueeze(0).transpose(0, 1)

        self.register_buffer('pe', pe)  ## 定一个缓冲区，其实简单理解为这个参数不更新就可以

    def forward(self, x):   ## 后续需要在进行一次输入 x 计算输出，当然提前定义好了  pos = PositionalEncoding(d_model)    output = pos(x)
        """
        x: [seq_len, batch_size, d_model]  pe: [max_len,1,d_model]    1 ==> batch_size  max_len ==> seq_len
        """
...
if __name__ == '__main__':
    ...
    # ich mochte ein bier P 为 编码器输入【P代表填充字符】   S i want a beer 为解码器输入【S代表开始标志】   解码器输出与 解码端真实标签i want a beer E 计算loss 【E代表结束标志】
    # 解码端不能并行，当输入为S，输出I，下一时刻的输入信息依赖于上一时刻的输出I ，为了加快训练速度 采用teacher forcing，把真实标签作为输入
    ...
    # 模型参数
    d_model = 512  # Embedding Size，每个字符转换为Embedding时 大小
    d_ff = 2048  # FeedForward dimension 前馈神经网络映射到Linear层有多少维度
    d_k = d_v = 64  # dimension of K(=Q), V
    n_layers = 6  # number of Encoder of Decoder Layer   6个Encoder和6个Decoder
    n_heads = 8  # number of heads in Multi-Head Attention  多头注意力机制分为8个头

    # 上述情况为batchsize=1，若batchsize=4，即每次输入四个句子，那么将padding_idx设置为8，大于长度8则删掉，小于长度8的则添加padding字符，负无穷

    # 写模型遵循两个规则： 1.从整体到局部   2.搞清楚数据的流动情况 输入-输出
    model = Transformer()
